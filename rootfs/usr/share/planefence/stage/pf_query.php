<?php
  ...
  if (strcmp($top . $hex . $call . $start . $end , "") == 0) {
	   echo "<html><body><H1>PlaneFence Query Interface</H1>";
	   echo "<h3>Usage: http://" . $_SERVER['SERVER_NAME'] . $_SERVER['REQUEST_URI'] . "?top=int&hex=regex&call=regex&start=regex&end=regex&type=csv|json</h3>";
	   ...
	   echo "<br />";
	   echo "For example:<br />";
	   echo "<b>http://" . $_SERVER['SERVER_NAME'] . $_SERVER['REQUEST_URI'] . "?top=5&hex=^A[DE]&start=2021/12/1[345]</b><br />";
	   echo "will return top 5 records of which the Hex ID starts with A followed by a D or E, and that have a start date/time that contains 2021/12/13, 2021/12/14, or 2021/12/15.<br />";
	   echo "<br />";
     ...
     system("/usr/share/planefence/pf_query.sh " . escapeshellarg($top) . " " . escapeshellarg($hex) . " " . escapeshellarg($call) . " " . escapeshellarg($start) . " " . escapeshellarg($end) . " " . escapeshellarg($outputtype) . " file=/usr/share/planefence/html/*.csv", $return_value );
